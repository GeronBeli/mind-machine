...
from utils.neural_search.pdf_reader import pdf_to_docVec
import logging
...
class FileSystemHandler:
        ...
        for file in files:
            ...
            with open(file_path, "wb") as f:
                ...
                try:              
                    ...
                    logging.debug(f"User {user_id} uploaded {file.filename}")
                    status_return.append([file.filename, True])
                except Exception as e:
                    logging.error(f"User {user_id} failed to upload {file.filename}: {e}")
    ...
    def encode_and_upload(self, file_path, user):
        ...
        logging.debug(f"User {user} is running OCR on {filename}")
        ocrmypdf.ocr(
            temp_file_path,
            temp_file_path,
            output_type='pdf',
        ...
        logging.debug(f"User {user} is encoding {filename}")
        ...
        if len(docVec.paras_vecs) <= 1 and docVec.paras_vecs[0]['paragraph'] == '':
            logging.error(f"User {user} failed to upload {filename}: No paragraphs recognized")
            raise Exception("No paragraphs recognized")  
        
        # save vectors to qdrant
        logging.debug(f"save {filename} vectors to qdrant")
    ...
    def delete_document(self, user_id, document_id):
        ...
        Returns:
            None
        """
        logging.debug(f"Attempting to delete document {document_id} for user {user_id}")
        if os.path.exists(self.document_directory + user_id + "/" + document_id):
            ...
            logging.info(f"Document {document_id} deleted for user {user_id}")
        else:
            logging.warning(f"Document {document_id} not found for user {user_id}")
    ...
    def edit_document_name(self, user_id, old_name, new_name):
        ...
        Returns:
            None
        """
        logging.debug(f"Attempting to rename document from {old_name} to {new_name} for user {user_id}")
        ...
        if os.path.exists(old_file_full_path):
            ...
            logging.info(f"Document renamed from {old_name} to {new_name} for user {user_id}")
        else:
            logging.warning(f"Document {old_name} not found for user {user_id}")
    ...
    def get_document_path(self, user_id, document_name):
        ...
        """
        logging.debug(f"Fetching document path for {document_name} for user {user_id}")
        path = self.document_directory + user_id + "/" + document_name
        if os.path.exists(path):
            logging.info(f"Document path for {document_name} retrieved for user {user_id}")
            return path
        else:
            logging.warning(f"Document {document_name} not found for user {user_id}")
            return False
    ...
    def file_system_exist(self, user_id):
        ...
        Returns:
            None
        """
        logging.debug(f"Checking if file system exists for user {user_id}")
        path = self.document_directory + user_id
        if not os.path.exists(path):
            os.makedirs(path)
            logging.info(f"File system created for user {user_id}")
    ...
    def convert_bytes(self, byte_size):
        ...
        logging.debug(f"Converting byte size: {byte_size}")
        unit_index = 0
    ...
    def get_last_modified_date(self, file_path):
        ...
        """
        logging.debug(f"Getting last modified date for file: {file_path}")
