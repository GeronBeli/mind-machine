...
class Qdrant:
    ...
    def check_user(self, userName):
        ...
        try:
            ...
            logging.debug(f"user: {userName} exists, and has {vectors_count} vectors")
        except:
            logging.debug(f"user doesn't exist, creating a new collection with the user name: {userName}")
    ...
    def add_docVec(self, userName, docVec):
        ...
        for para in docVec.paras_vecs:
            logging.debug(f"Vector Count: {vectors_count} Paragraph: {para['paragraph']}")
    ...
    def get_hits(self, collection_name, search_text):
        ...
        """
        logging.debug(f"Searching for {search_text} in {collection_name}")
    ...
    def get_scores(self, hits):
        ...
        """
        logging.debug(f"Getting scores from search hits")
        max_score_value = -1
        max_score_value_indoc = None
        score_values_indoc = []
        for hit in hits:
            logging.debug(f"Payload: {hit.payload} Score: {hit.score}")
    ...
    def delete_doc(self, collection_name, doc_name):
        ...
        None
        """
        logging.debug(f"Deleting {doc_name} from {collection_name}")
    ...
    def revectorize_all(self):
        ...
        """
        logging.debug(f"Revectorizing all documents")
        ...
        for user in all_users:
            ...
            for file in files:
                logging.debug(f"Encoding and uploading {os.path.join(config.document_directory, user, file['file_name'])}")
                file_path = os.path.join(config.document_directory, user, file['file_name'])
                try:
                    fileHandler.encode_and_upload(file_path, user)
                except:
                    logging.debug(f"No text recognized in {file_path}")
    ...
    def rename_doc(self, collection_name, doc_name, new_name):
        ...
        """
        logging.debug(f"Renaming {doc_name} to {new_name} in {collection_name}")
